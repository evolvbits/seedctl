//! BNB Smart Chain watch-only wallet export builder.
//!
//! Thin wrapper around [`seedctl_core::evm::build_watch_only_export`] bound
//! to the [`seedctl_core::evm::BNB_PROFILE`] so that the rest of the crate
//! never references the profile constant directly.

use seedctl_core::export;
use std::error::Error;

/// Builds a [`export::WalletExport`] document for a BNB Smart Chain
/// watch-only wallet.
///
/// Delegates to [`seedctl_core::evm::build_watch_only_export`] with the
/// [`seedctl_core::evm::BNB_PROFILE`], which fills in the BNB-specific
