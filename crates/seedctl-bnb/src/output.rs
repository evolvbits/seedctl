//! BNB Smart Chain wallet output rendering.
//!
//! Thin wrapper around [`seedctl_core::evm::print_wallet_output`] that binds
//! the [`seedctl_core::evm::BNB_PROFILE`] so the rest of the crate never
//! references the profile constant directly.

/// All data required to render a BNB Smart Chain wallet section to stdout.
///
...
/// Renders the full BNB Smart Chain wallet section to stdout.
///
/// Delegates to [`seedctl_core::evm::print_wallet_output`] with the
/// [`seedctl_core::evm::BNB_PROFILE`], which supplies the wallet title,
/// export network identifier, and scan paths specific to BNB Smart Chain.
///
